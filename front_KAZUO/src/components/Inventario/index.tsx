...
export default function Inventario() {
  ...
  const fetchProducts = async () => {
    try {
      ...
      // Podrías establecer un estado de error para mostrar un mensaje al usuario.
    }
  };
  ...
  const handleImageUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      try {
        const url = `${kazuo_back}/files/uploadProfileImage`;  // Verifica si la URL está bien
        const response = await fetch(url, {
          method: 'POST',
          headers: {
            "Authorization": `Bearer ${userData?.token}`, // Asegúrate de usar el formato correcto
          },
          body: formData,
        });
      ...
      <div className="flex justify-between items-center mb-8">
        ...
        <div className="space-x-4">
          ...
          <button 
            ...
            onClick={()=>  handleNavigateToCreateProduct ("/storeform") } // Cambia la ruta según necesites
          >
            Crear Bodega 
          </button>
        </div>
