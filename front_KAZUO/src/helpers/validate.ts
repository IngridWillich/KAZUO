...
export function validateProductForm(values: IProduct): IProductsErrors {
  ...
  if (!values.categoryName) {
      errors.categoryName = "El ID de la bodega es obligatorio";
  } else if (!/^[A-Za-z0-9]+$/.test(values.categoryName)) {
      errors.categoryName = "El ID de la bodega solo puede contener letras y números";
  }

  // Validación de la imagen
  if (!values.imgUrl) {
      errors.imgUrl = "La imagen es obligatoria";
  } else if (!/\.(jpg|jpeg|png|gif)$/i.test(values.imgUrl)) {
      errors.imgUrl = "La imagen debe ser en formato jpg,png ";
  }
